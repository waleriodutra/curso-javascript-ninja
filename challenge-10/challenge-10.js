//(
    ...
    */
    ...
    function isOperatorValid(operador){
        return (operador === '+' || operador === '-' ||operador === '*' || 
                    operador === '/' ||operador === '%');
        /* Uma forma amis inteligente
            return (operation[operador] !== undefined);
        */
        /* Truthy
            retur     n !!operation[operador]
        */
    ...
    */
    function calculator(operator){
        var validador = isOperatorValid(operator);
        if(validador){
            return function(numa, numb){
                var teste = (typeof numa === 'number' && typeof numb === 'number');
                if(teste){
                    return operation[operator](numa, numb);
                }
                return teste;
            }
        }
    ...
    */
    function showOperationMessage(operador, numa, numb){
        return 'A operação ' + numa + ' ' + operador + ' ' + numb + ' =';
    }
    ...
    */
    function showErrorMessage(operCalculo){
        return 'Operação ' + operCalculo + ' não permitida!';
    }
    ...
    */
    ...
    function calculoFinal(variavelOperador){
        if (!variavelOperador){
            console.log(showErrorMessage(operationSignal));
        } else {
            number1 = 13;
            number2 = 3;
            console.log(showOperationMessage(operationSignal, number1, number2), variavelOperador(number1, number2).toFixed(1));
        }
    }
    ...
    */
    ...
    operationSignal = '%';
    var modu = calculator(operationSignal);
    calculoFinal(modu);
    ...
    */
    operationSignal = 'UACAUACA';
    var invalido = calculator(operationSignal);
    calculoFinal(invalido);
